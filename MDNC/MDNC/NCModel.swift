...
class NCModel {
    ...
    static let myMenuChangedNotification = Notification.Name("MyMenuChanged")
    
    let menu: NCMenu
    
    var myMenu: [NCFood] = [] {
        didSet { NotificationCenter.default.post(name: NCModel.myMenuChangedNotification, object: nil) }
    }
